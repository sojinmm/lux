defmodule Lux.Lenses.Etherscan.DailyUncleBlockCount do
  ...
  use Lux.Lens,
    name: "Etherscan.DailyUncleBlockCount",
    description: "Tracks orphaned blocks and their rewards over time (requires Pro API key)",
    ...
    schema: %{
      ...
      properties: %{
        ...
        startdate: %{
          type: :string,
          description: "Beginning date for uncle block data in yyyy-MM-dd format"
        },
        enddate: %{
          type: :string,
          description: "Ending date for uncle block data in yyyy-MM-dd format"
        },
        sort: %{
          type: :string,
  ...
  def before_focus(params) do
    ...
    |> Map.put(:action, "dailyuncleblkcount")
      ...
      {:ok, %{result: result}} when is_list(result) ->
        ...
        processed_results = Enum.map(result, fn item ->
          %{
            ...
            uncle_blocks_count: Map.get(item, "uncleBlockCount", ""),
            uncle_blocks_rewards_eth: Map.get(item, "uncleBlockRewards", "")
          }
        end)
