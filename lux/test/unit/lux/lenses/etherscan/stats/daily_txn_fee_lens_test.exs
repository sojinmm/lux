defmodule Lux.Lenses.Etherscan.DailyTxnFeeLensTest do
  ...
  describe "focus/1" do
    test "makes correct API call and processes the response with ascending sort" do
      ...
      Req.Test.expect(Lux.Lens, fn conn ->
        ...
        assert query["action"] == "dailytxnfee"
        ...
        Req.Test.json(conn, %{
          ...
          "result" => [
            %{
              "UTCDate" => "2023-01-01",
              "transactionFee_Eth" => "123.45"
            },
            %{
              "UTCDate" => "2023-01-02",
              "transactionFee_Eth" => "234.56"
            },
            %{
              "UTCDate" => "2023-01-03",
              "transactionFee_Eth" => "345.67"
            }
          ]
        })
      end)
      ...
      assert length(txn_fee_data) == 3

      # Verify first day's data
      first_day = Enum.at(txn_fee_data, 0)
      assert first_day.utc_date == "2023-01-01"
      assert first_day.tx_fee_eth == 123.45

      # Verify second day's data
      second_day = Enum.at(txn_fee_data, 1)
      assert second_day.utc_date == "2023-01-02"
      assert second_day.tx_fee_eth == 234.56

      # Verify third day's data
      third_day = Enum.at(txn_fee_data, 2)
      assert third_day.utc_date == "2023-01-03"
      assert third_day.tx_fee_eth == 345.67
    end
    ...
    test "makes correct API call with descending sort" do
      ...
      assert length(txn_fee_data) == 3
      assert Enum.at(txn_fee_data, 0).utc_date == "2023-01-03"
      assert Enum.at(txn_fee_data, 2).utc_date == "2023-01-01"
    end
  ...
  describe "before_focus/1" do
    test "prepares parameters correctly with defaults" do
      ...
      assert result.action == "dailytxnfee"
    ...
    test "raises error when Pro API key is not available" do
      ...
      assert_raise ArgumentError, "This endpoint requires an Etherscan Pro API key.", fn ->
        DailyTxnFee.before_focus(params)
      end
    end
  end
  ...
  describe "after_focus/1" do
    test "processes successful response" do
      ...
      # Verify the result
      assert {:ok, %{result: txn_fee_data, daily_txn_fee: txn_fee_data}} = result
      assert length(txn_fee_data) == 2
      assert Enum.at(txn_fee_data, 0).utc_date == "2023-01-01"
      assert Enum.at(txn_fee_data, 0).tx_fee_eth == 123.45
      assert Enum.at(txn_fee_data, 1).utc_date == "2023-01-02"
      assert Enum.at(txn_fee_data, 1).tx_fee_eth == 234.56
    end
