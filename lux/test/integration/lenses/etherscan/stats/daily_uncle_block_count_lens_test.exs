defmodule Lux.Integration.Etherscan.DailyUncleBlockCountLensTest do
  ...
  defmodule NoAuthDailyUncleBlockCountLens do
    @moduledoc """
    Going to call the api without auth so that we always fail
    """
    use Lux.Lens,
      name: "Etherscan Daily Uncle Block Count API",
      description: "Fetches the daily uncle block count",
    ...
    def before_focus(params) do
      ...
      |> Map.put(:action, "dailyuncleblockcount")
    end
  end
  ...
  test "can fetch daily uncle block count with required parameters" do
    ...
    if not has_pro_api_key?() do
      ...
      assert {:ok, %{result: uncle_block_data}} =
               RateLimitedAPI.call_standard(DailyUncleBlockCount, :focus, [%{
                 startdate: @start_date,
                 enddate: @end_date,
                 chainid: 1
               }])
  ...
  test "fails when no auth is provided" do
    # The NoAuthDailyUncleBlockCountLens doesn't have an API key, so it should fail
    result = RateLimitedAPI.call_standard(NoAuthDailyUncleBlockCountLens, :focus, [%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    }])
  ...
  test "raises error or returns error for Pro API endpoint" do
    ...
    result = RateLimitedAPI.call_standard(DailyUncleBlockCount, :focus, [%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    }])
