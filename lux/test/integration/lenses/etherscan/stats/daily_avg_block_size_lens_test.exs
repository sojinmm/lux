defmodule Lux.Integration.Etherscan.DailyAvgBlockSizeLensTest do
  ...
  test "can fetch daily average block size with required parameters" do
    ...
    if not has_pro_api_key?() do
      ...
      assert {:ok, %{result: block_size_data}} =
               RateLimitedAPI.call_standard(DailyAvgBlockSize, :focus, [%{
                 startdate: @start_date,
                 enddate: @end_date,
                 chainid: 1
               }])
  ...
  test "fails when no auth is provided" do
    ...
    result = RateLimitedAPI.call_standard(NoAuthDailyAvgBlockSizeLens, :focus, [%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    }])
  ...
  test "raises error or returns error for Pro API endpoint" do
    ...
    result = RateLimitedAPI.call_standard(DailyAvgBlockSize, :focus, [%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    }])
