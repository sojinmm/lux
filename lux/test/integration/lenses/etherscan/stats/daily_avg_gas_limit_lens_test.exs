defmodule Lux.Integration.Etherscan.DailyAvgGasLimitLensTest do
  ...
  alias Lux.Lenses.Etherscan.DailyAvgGasLimitLens
  ...
  defp has_pro_api_key? do
    ...
    case DailyAvgGasLimitLens.focus(%{
  ...
  test "can fetch daily average gas limit with required parameters" do
    ...
    if not has_pro_api_key?() do
      IO.puts("Skipping test: Pro API key required for DailyAvgGasLimitLens or invalid action name")
      :ok
    else
      assert {:ok, %{result: gas_limit_data}} =
               DailyAvgGasLimitLens.focus(%{
                 startdate: @start_date,
                 enddate: @end_date,
                 chainid: 1
               })
  ...
  test "fails when no auth is provided" do
    ...
    result = NoAuthDailyAvgGasLimitLens.focus(%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    })
  ...
  test "raises error or returns error for Pro API endpoint" do
    ...
    result = DailyAvgGasLimitLens.focus(%{
      startdate: @start_date,
      enddate: @end_date,
      chainid: 1
    })
