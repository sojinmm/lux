defmodule Lux.Memory.SimpleMemoryTest do
  ...
  describe "memory operations" do
    ...
    test "retrieves entries by time window", %{pid: pid} do
      ...
      {:ok, _} = SimpleMemory.add(pid, "before window", :observation, %{timestamp: DateTime.to_unix(DateTime.add(start_time, -3600))})
      {:ok, _} = SimpleMemory.add(pid, "in window 1", :observation, %{timestamp: DateTime.to_unix(DateTime.add(start_time, 1))})
      {:ok, _} = SimpleMemory.add(pid, "in window 2", :observation, %{timestamp: DateTime.to_unix(DateTime.add(start_time, 1800))})
      {:ok, _} = SimpleMemory.add(pid, "after window", :observation, %{timestamp: DateTime.to_unix(DateTime.add(start_time, 7200))})

      end_time = DateTime.add(start_time, 3600)

      assert {:ok, entries} = SimpleMemory.window(pid, start_time, end_time)
      assert length(entries) == 2
      assert Enum.map(entries, & &1.content) |> Enum.sort() == ["in window 1", "in window 2"]
    end
